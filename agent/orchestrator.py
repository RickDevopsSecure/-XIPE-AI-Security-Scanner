...
class PentestOrchestrator:
    ...
    def _deduplicate_findings(self) -> list:
        """Consolida hallazgos duplicados (mismo título + categoría + módulo)."""
        seen = {}
        deduped = []
        for f in self.all_findings:
            ...
            if key not in seen:
                ...
                # Actualizar evidencia si la nueva es más rica
                existing = seen[key]
    ...
    def _generate_outputs(self):
        ...
        # Deduplicar antes de reportar
        deduplicated = self._deduplicate_findings()
        removed = len(self.all_findings) - len(deduplicated)
        if removed > 0:
            self.logger.info(f"Deduplicación: {removed} hallazgo(s) duplicado(s) consolidados.")
